...
const NtsApplicationForm = ({
  ...
  const handleSubmit = async (
    ...
    setLoading(true);
    try {
      const createPayload: NtsApplicationManager = {
        title: values.title,
      ...
      if (values.id) {
        ...
        resp = await createNewApplication(createPayload);
      }
      ...
      if (submitType === "save-close") {
        router.push("/ntsapplication-manager");
      } else if (!values.id) {
        router.push(`/ntsapplication-manager/${saved.id}`);
      }
