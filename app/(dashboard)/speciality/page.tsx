import { bulkDeleteSpecialitys, getAllSpecialitys } from "@/app/actions/speciality.actions";
...
export default async function Page( {searchParams} : SearchParams ) {
    ...
    const { data, totalRecords } = await getAllSpecialitys({
        ...
        <>
            <div className="flex items-center ">
                <div className="ml-auto flex items-center gap-4">
                    ...
                    <AddNewLinkButton href="/speciality/add" />
                </div>
            </div>
            ...
            <div className="overflow-hidden">
                <Suspense fallback={<Loading />}>
                    <CustomDataTable
                        heading="Specialitys"
                        subHeading="Manage your Specialitys here."
                        columns={specialityColumns}
                        data={data}
                        rowCount={totalRecords}
                        deleteServerAction={bulkDeleteSpecialitys}
                        page={resolvedSearchParams?.page}
                    />
                </Suspense>
            </div>
